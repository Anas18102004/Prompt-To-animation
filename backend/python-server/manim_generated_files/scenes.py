...
class Introduce(Scene):
    def construct(self):
        ...
        back_arrow_1 = DashedLine(first_output_node.get_center(), first_hidden_node.get_center(), buff=0.5, color=RED)
        back_arrow_2 = DashedLine(first_hidden_node.get_center(), input_node.get_center(), buff=0.5, color=RED)
        ...
        ball = Dot(point=hill.get_points()[0], color=BLUE)
        ...
        self.play(ball.animate.move_to(hill.get_points()[int((new_x + 2)*(10/4))]),run_time=0.5)
        self.wait(3)
        ...
        axes = Axes(
            x_range=[0, 10, 1],
            y_range=[0, 1, 0.2],
            axis_config={"label_constructor": Text, "include_numbers": False}
        )
        x_label = Text("Epochs").next_to(axes.x_axis, RIGHT)
        y_label = Text("Error").next_to(axes.y_axis, UP)
